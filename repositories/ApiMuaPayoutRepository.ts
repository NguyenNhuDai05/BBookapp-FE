...
export class ApiMuaPayoutRepository implements IMuaPayoutRepository {
  ...
  async addBankAccount(request:UpsertMuaBankAccountRequest){const {data}=await api.post('/mua/bank-accounts',{...request,qrCodeUrl:request.qrCodeUrl||undefined});return mapBank(data);}
  async updateBankAccount(id:string,request:UpsertMuaBankAccountRequest){const {data}=await api.put(`/mua/bank-accounts/${id}`,{...request,qrCodeUrl:request.qrCodeUrl||undefined});return mapBank(data);}
